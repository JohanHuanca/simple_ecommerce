...
const getNotificationClasses = (type: NotificationType) => {
  const base = 'animate-bounce fixed bottom-5 right-5 z-50 transition-all duration-300 ease-in-out p-4 rounded-lg shadow-lg max-w-sm flex items-center';
  switch (type) {
...
const Notification: React.FC<NotificationProps> = ({ message, type, duration = 3000, onClose }) => {
  ...
  return (
    <div className={getNotificationClasses(type)}>
      ...
      <button
        ...
        className="flex-shrink-0 ml-3 text-white hover:text-gray-200 focus:outline-none close-button"
      >
